<?php
/*
* Template Name: 東京奄美会メニュー（公開用－ＮＥＷ）
*/
...
if (!empty($filtered_posts)) {
    ...
    foreach ($categories_with_posts as $cat_data) {
        ...
        // カテゴリ見出し: 明るいブルー背景＋太字白文字
        echo '<section style="margin-bottom:20px;">';
        echo '<h2 style="text-align:center; background:linear-gradient(90deg,#6ec6ff 0%,#2196f3 100%); color:#fff; font-weight:bold; padding:12px 0; border-radius:18px; margin-bottom:10px; letter-spacing:1px; font-size:20px; text-shadow:1px 1px 4px rgba(0,0,0,0.18);">' . esc_html($category->name) . '</h2>';
        ...
        $max_cols = 2;
        $max_rows = 2;
        $total = count($posts_with_menu_no);
        $cells = [];
        foreach($posts_with_menu_no as $post_data){
            ...
            $menu_link = home_url("/menu-detail-public/?menu_no=".$menu_no);
            ...
            $cell = '<a href="'.esc_url($menu_link).'" style="display:block; padding:6px 4px; background:#fff; color:#1e90ff; border-radius:8px; border:2px solid '.$border_color.'; text-decoration:none; font-weight:bold; font-size:15px;">'.$display_title.'</a>';
            if($is_admin){
                $edit_link = get_edit_post_link($post_id);
                $cell .= '<a href="'.esc_url($edit_link).'" style="display:inline-block; margin-top:2px; padding:2px 6px; background-color:#4facfe; color:#ffffff; font-weight:bold; border-radius:4px; text-decoration:none; font-size:11px;">編集</a>';
            }
            $cells[] = $cell;
        }
        echo '<table style="width:100%; max-width:700px; margin:0 auto 10px auto; border-collapse:separate; border-spacing:8px 6px;">';
        $cell_idx = 0;
        for($row=0; $row<$max_rows; $row++){
            echo '<tr>';
            for($col=0; $col<$max_cols; $col++){
                if($cell_idx < $total){
                    echo '<td style="width:50%; vertical-align:top; text-align:center; padding:0;">'.$cells[$cell_idx].'</td>';
                }else{
